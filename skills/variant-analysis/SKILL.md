...
- Initial vulnerability discovery — use the existing debugging/review workflow
- General code review with no known pattern to search for
- Writing fix recommendations — use the existing project review/reporting workflow
- Understanding unfamiliar code — first establish source context with the existing project workflow
...
## Codex routing and scope

Follow the five steps directly. The upstream Claude plugin command and its named subagents are not installed. Use the existing bounded Codex collaboration workflow only when independent search axes justify it. Stay within the requested repositories and report proposed regression checks; a search request alone does not authorize CI changes or fixes. Optional CodeQL/Semgrep examples require those tools to be available; use `rg` for initial exact searches.
