...
metadata:
  ...
  upstream-last-updated: "2026-09-08"
  tags: "frontend, design, ui, ux, accessibility, responsive"
description: "Design or substantially revise frontend interfaces with deliberate visual direction and rendered verification. Also use for frontend performance diagnosis, release-quality audits, and browser regression strategy; select only the relevant mode, not a full redesign."
...
For a focused performance, release-quality or browser-regression task, read the
applicable [quality mode](./references/quality-modes.md) and verify that outcome.
Do not require a new visual thesis, redesign or whole-page certification for
such work. A bounded prototype may leave production states unfinished if they
are identified as such; assess what it teaches before deciding what to harden.
...
- [Optional Quality Modes](./references/quality-modes.md): read only when layout direction is materially uncertain, a user-visible performance or release-quality question needs evidence, or a reproduced rendered journey justifies the first browser regression. Do not apply it as a routine checklist for ordinary frontend work.
...
- Codex: install or sync the folder into
  `{{SKILLS}}/frontend-design` and restart Codex after major changes.
...
Use the installed browser/computer-use, visualization, and project-specific skills when relevant. No sibling skills from this upstream bundle are required or installed.

## Contrast helper limits

Use the helper for explicitly identified opaque foreground/background pairs, with `#RGB` or `#RRGGBB` values. On Windows use `py -3 -X utf8` or the active bundled interpreter. The `--batch` mode compares every palette combination without knowing which colors touch; its output is exploratory and its exit code is not a WCAG gate. Alpha compositing, gradients, images, CSS variables, font size/weight, and actual rendered states require separate inspection. A contrast result alone does not establish accessibility conformance.
