...
description: "Assess scientific claims, ML or game-AI benchmark gains, causal comparisons, experimental designs, and research papers for confounding, selection bias, leakage, and uncertainty. Use when judging what evidence supports; not mechanical data cleanup or routine implementation."
license: MIT license
metadata:
  version: "1.3"
...
Evaluate the specific scientific or benchmark claim using methodology, design, uncertainty, bias, and confounding evidence. For ML and simulation comparisons, start with the estimand, matched conditions, selection process, and untouched evaluation data. Use clinical grading frameworks only for questions they were designed to answer.
...
Choose the framework from the claim: predictive accuracy, causal effect, diagnostic accuracy, evidence synthesis, or exploratory hypothesis. GRADE and clinical hierarchies are not universal scoring systems for ML/game experiments. For numerical claims, identify the estimand, independent unit, effect size, uncertainty or missing uncertainty, paired/grouped design, multiple comparisons, and untouched evaluation data. State what is unavailable instead of fabricating an interval. Distinguish inference uncertainty, external validity, and practical decision risk.

Use existing local visualization tools if a figure would help. This installation has no external schematic service or API requirement.
...
Adapted from K-Dense Scientific Agent Skills; upstream commit and license are recorded in `SOURCE.json` and `UPSTREAM-LICENSE.md`. Cite primary scientific sources for the claims in the user's output. Do not automatically add a tool/vendor paper to manuscripts or reports.
